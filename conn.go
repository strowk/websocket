...
func Dial(url string) (net.Conn, error) {
	return DialWithSubprotocols(url, nil)
}

// TODO(nightexcessive): Add a Dial function that allows a deadline to be
// specified.

// DialWithSubprotocols opens a new WebSocket connection with specified subprotocols.
// It will block until the connection is established or fails to connect.
func DialWithSubprotocols(url string, subprotocols []string) (net.Conn, error) {
	ws, err := websocketjs.NewWithSubprotocols(url, subprotocols)
	if err != nil {
		return nil, err
	}
	conn := &conn{
